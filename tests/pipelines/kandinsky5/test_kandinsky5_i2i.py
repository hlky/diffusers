...
class Kandinsky5I2IPipelineFastTests(PipelineTesterMixin, unittest.TestCase):
    ...
    def test_precomputed_prompt_embeds_batching_with_tensor_image(self):
        device = "cpu"
        components = self.get_dummy_components()
        pipe = self.pipeline_class(**components)
        pipe.resolutions = [(64, 64)]
        ...
        latents = torch.zeros(
            ...
            1,
            64 // pipe.vae_scale_factor_spatial,
            64 // pipe.vae_scale_factor_spatial,
            2 * components["transformer"].config.in_visual_dim + 1,
        )
        expected_latent_markers = torch.arange(1, batch_size * num_images_per_prompt + 1, dtype=latents.dtype)
        ...
        self.assertEqual(output.images.shape[0], batch_size * num_images_per_prompt)
        self.assertEqual(len(captured_inputs), 2)
        torch.testing.assert_close(
            captured_inputs[0]["encoder_hidden_states"],
            prompt_embeds_qwen.repeat(1, num_images_per_prompt, 1).view(
                batch_size * num_images_per_prompt, sequence_length, -1
            ),
        )
        torch.testing.assert_close(
            captured_inputs[1]["encoder_hidden_states"],
            negative_prompt_embeds_qwen.repeat(1, num_images_per_prompt, 1).view(
                batch_size * num_images_per_prompt, sequence_length, -1
            ),
        )
        torch.testing.assert_close(
            captured_inputs[0]["pooled_projections"],
            prompt_embeds_clip.repeat_interleave(num_images_per_prompt, dim=0),
        )
        torch.testing.assert_close(
            captured_inputs[1]["pooled_projections"],
            negative_prompt_embeds_clip.repeat_interleave(num_images_per_prompt, dim=0),
        )
...
@slow
@require_torch_accelerator
class Kandinsky5I2IPipelineIntegrationTests(unittest.TestCase):
    def setUp(self):
        super().setUp()
        gc.collect()
    ...
    def test_kandinsky5_i2i_lite_sft(self):
        pipe = Kandinsky5I2IPipeline.from_pretrained(
            "kandinskylab/Kandinsky-5.0-I2I-Lite-sft-Diffusers", torch_dtype=torch.bfloat16
        )
        ...
        output = pipe(
            ...
            prompt="Turn the red square into a clean studio product photo",
            negative_prompt="",
            height=1024,
            width=1024,
            num_inference_steps=1,
