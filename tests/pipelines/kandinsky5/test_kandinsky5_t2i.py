...
class Kandinsky5T2IPipelineFastTests(PipelineTesterMixin, unittest.TestCase):
    ...
    def test_precomputed_prompt_embeds_batching(self):
        ...
        latents = torch.zeros(
            ...
            64 // pipe.vae_scale_factor_spatial,
            components["transformer"].config.in_visual_dim,
        )
...
@slow
@require_torch_accelerator
class Kandinsky5T2IPipelineIntegrationTests(unittest.TestCase):
    def setUp(self):
        super().setUp()
        gc.collect()
    ...
    def test_kandinsky5_t2i_lite_sft(self):
        pipe = Kandinsky5T2IPipeline.from_pretrained(
            "kandinskylab/Kandinsky-5.0-T2I-Lite-sft-Diffusers", torch_dtype=torch.bfloat16
        )
        pipe.enable_model_cpu_offload(device=torch_device)
        pipe.set_progress_bar_config(disable=None)

        output = pipe(
            prompt="A clean studio product photo of a red square",
            negative_prompt="",
            height=1024,
            width=1024,
            num_inference_steps=1,
